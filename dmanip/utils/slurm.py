...
def random_string(
    n, include_lowercase=True, include_uppercase=False, include_digits=False
):
    alphabet = ""
...
def parse_squeue():
    squeue_out = subprocess.run(
        ["squeue", "-u", Path.home().name, "-o", "%.10i %.25j %.2t %.10M %.6D %R"],
    ...
    for group_id, group_data in metadata.items():
        for job_id in list(group_data["active jobs"].keys()):
            if job_id not in active_job_ids:
                ...
                group_data["completed jobs"][job_id] = group_data["active jobs"].pop(
                    job_id
                )
...
def print_metadata():
    ...
    ids_by_time = [
        k for k, v in sorted(metadata.items(), key=lambda item: item[1]["timestamp"])
    ]
