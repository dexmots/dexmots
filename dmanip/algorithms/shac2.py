...
class SHAC:
    def __init__(self, cfg):
        ...
        if "stochastic_init" in cfg["params"]["diff_env"]:
            stochastic_init = cfg["params"]["diff_env"]["stochastic_init"]
            cfg["params"]["diff_env"].pop("stochastic_init")
        else:
        ...
        self.early_stopping_patience = cfg["params"]["config"].get("early_stopping_patience", self.max_epochs)
        ...
        def actor_closure():
            ...
            with torch.no_grad():
                ...
                if torch.isnan(self.grad_norm_before_clip) or self.grad_norm_before_clip > 1000000.0:
                    print("shac training crashed due to unstable gradient, saving checkpoint")
