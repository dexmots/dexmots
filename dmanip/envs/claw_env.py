...
from ..utils.common import ORIENTATION_GOAL_TYPES, POSITION_GOAL_TYPES, ActionType, IntegratorType, ObjectType, GoalType, RewardType, to_numpy, joint_coord_map
from ..utils.common import supported_joint_types, clear_state_grads
from ..utils.autograd2 import get_compute_graph
...
class ClawWarpEnv(ObjectTask):
    ...
    sim_substeps_xpbd = 5
    termination_threshold = np.pi * 4 / 5
    ...
    def __init__(
        ...
        self.graph_capture_params['reward_ctrl'] = self.reward_ctrl
        self.graph_capture_params['reward_total'] = self.reward_total
        self.graph_capture_params['reward_vec_prev'] = self.reward_vec_prev
        self.graph_capture_params['reward_vec'] = self.reward_vec
        self.state_list = self.simulate_params.get("state_list", None)
        self.act_params["q_offset"] = 1
    ...
    def create_articulation(self, builder):
        ...
        if self.goal_type in ORIENTATION_GOAL_TYPES:
            builder.joint_q[1:] = [0.824, -1.365, -0.824, 1.365]
        # else:
        ...
        hand_env_joint_mask = list(
                map(lambda x: x[0], 
                    filter(lambda x: x[1] in valid_joint_types and 
                           "joint" in self.hand_joint_names[x[0]], 
                           enumerate(builder.joint_type)
                           )
                    )
                )
        ...
        self.env_num_joints = len(hand_env_joint_mask) 
        self.env_joint_target_indices = hand_env_joint_mask
        self.object_num_joint_q = len(joint_indices)
        self.object_joint_target_indices = joint_indices
    ...
    def post_finalize(self):
        ...
        if self.integrator_type == IntegratorType.EULER:
            self.model.joint_attach_ke = 1e2
        ...
        start_joint_q = wp.to_torch(self.model.joint_q).detach()
        start_joint_qd = wp.to_torch(self.model.joint_qd).detach()
        joint_X_p = wp.to_torch(self.model.joint_X_p).detach()
        ...
        self.start_joint_stiffness_pt = wp.to_torch(self.start_joint_stiffness).view(self.num_envs, -1).detach()
    ...
    def load_goal_trajectory(self, goal_path=None, env_ids=None):
        """Loads goal trajectory from an existing file or generates a new one."""
        if env_ids is None:
    ...
    def calculate_reward_kernels(self, goal_state_vars):
        """Launches warp kernels for computing rewards using goal_state_vars"""
        wp.launch(
    ...
    def forward_simulate(self, joint_act):
        ...
        for step in range(self.sim_substeps):
            if not self.requires_grad:
                state_out, state_in = state_in, state_out
            else:
                state_in = state_out
                if step == self.sim_substeps - 1:
                    state_out = self.state_1
                else:
                    state_out = self.state_list[step]
            state_in.clear_forces()
            # updates ground/rigid contacts
            if not self.use_graph_capture and not self.requires_grad:
        ...
        self.state_0, self.state_1 = state_out, state_in
    ...
    def calculateReward(self):
        ...
        if self.use_graph_capture:
            if self.calculate_reward_graph is None:
                self.calculate_reward_graph = get_compute_graph(self.calculate_reward_kernels, kwargs=dict(goal_state_vars=goal_state_vars))
        ...
        rewards = torch.as_tensor(self.reward_vec.numpy().copy(), device=self.device).view(self.num_envs, -1)
    ...
    def clear_grad(self, checkpoint=None):           
        ...
        with torch.no_grad():
            if hasattr(self, "joint_q") and self.joint_q is not None:
    ...
    def assign_scale_actions(self, actions):
        """Computes actions according to action_type and additionally returns 
        corresponding joint_act warp array to be set"""
        if self.action_type is ActionType.TORQUE:
            ...
            actions = (jll - jlu) * (actions + 1.0) / 2.0 + jlu
    ...
    def warp_step(self, actions, requires_grad=False):
        """Steps integrator for num_substeps"""
        if requires_grad:
    ...
    def _check_early_termination(self):
        ...
        self.extras['termination'] = termination_buf = too_far_away.flatten()
    ...
    def step(self, actions):
        ...
        self.reset_buf = self.reset_buf | self._check_early_termination()
        ...
        if self.requires_grad:
            ...
            self.extras["episode_end"] = self.extras["termination"] = self.termination_buf
            self.extras["truncation"] = (self.progress_buf >= self.episode_length)
